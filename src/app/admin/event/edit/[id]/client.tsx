...
import { updateEvent } from "@/actions/admin/events";
...
import DatePickerInput from "../../register/DatePickerInput";
import TimeInput from "../../register/TimeInput";
import { MapPin } from 'lucide-react';
import { Textarea } from "@/components/ui/textarea";
import { RadioGroup, RadioGroupItem } from "@/components/ui/radio-group";
import MemberTable from "../../register/MemberTable";
import MemberDialog from "../../register/MemberDialog";
import GuestTable from "../../register/GuestTable";
import { Button } from "@/components/ui/button";
import { CirclePlus } from 'lucide-react';
import { useForm, Controller, FormProvider } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { eventFormSchema, EventFormValues } from "../../register/validation";
...
// 日付をHH:mm形式に変換するユーティリティ関数
const formatTime = (date: Date) => {
  return format(date, 'HH:mm');
};

export default function EventEditClientPage({ event }: { event: Event }) {
    const id = useId();
  ...
  const defaultValues: Partial<EventFormValues> = {
    ...
    ticketType: event.ticket_price > 0 ? "paid" : "free",
    price: event.ticket_price > 0 ? event.ticket_price.toString() : "",
    capacityType: event.capacity ? "limited" : "unlimited",
    capacity: event.capacity ? event.capacity.toString() : "",
    members: [],
    eventImage: event.image_url || "",
    is_published: event.is_published,
    startDate: new Date(event.start_datetime),
    startTime: formatTime(new Date(event.start_datetime)),
    endDate: new Date(event.end_datetime),
    endTime: formatTime(new Date(event.end_datetime))
  };
    ...
    const onSubmit = async (data: EventFormValues) => {
      ...
      try {
        ...
        console.log("updateEvent 呼び出し後", response);
        ...
        if (response.success) {
          ...
          // イベント詳細ページへリダイレクト
          router.push(`/admin/event/edit/success?id=${event.id}`);
        } else {
          // エラー処理
          console.error('イベント更新エラー:', response.error);
          alert(`イベント更新に失敗しました: ${response.message}`);
        }
      } catch (error) {
        console.error('イベント更新エラー:', error);
        alert('イベント更新中にエラーが発生しました');
      } finally {
