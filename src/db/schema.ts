...
export const invitationCodes = pgTable(
  ...
  {
    id: serial("id").primaryKey(),
    // 実際にユーザーが入力する「招待コード文字列」
    code: varchar("code", { length: 255 }).notNull(),

    organization_id: integer("organization_id").notNull(),
    // operator招待ならイベント不要、ゲスト招待の場合などにイベントに紐づけたい場合はnull許容にする
    event_id: integer("event_id"), // nullable

    // "operator_invitation" | "guest_invitation" | ...など拡張しやすい形でOK
  ...
  (t) => ({
    // コード文字列で一意制約（被り防止）
    uniqueCode: unique("unique_code").on(t.code),
  })
);
